...
def tab2alm(tab):
    ...
    lmax = np.int64(np.shape(tab)[0])-1
    taille = np.int64(lmax*(lmax+3)/2)+1
    alm = np.zeros((taille,),dtype=complex)

    for r in range(0,taille):
        l,m = hpy.sphtfunc.Alm.getlm(lmax,r)
        alm[r] = np.complex128(tab[l,m,0] + 1j * tab[l,m,1])
        #alm[r] = np.complex128(tab[l,m,0],tab[l,m,1])
...
def get_one_face(Imag,NumFace=0,nested=False):
    ...
    taille_face = npix/12
    cote=np.int64(np.sqrt(taille_face))
    if (nested!=True):
    ...
    try:
        ...
        for kreg in range(len(dict_match)):
            ...
            if keep_reso is True:
                ...
                nxpix=np.int64(max([np.ceil(dict_match[kreg]["dl"]*60./reso_arcmin),minpix]))
                nypix=np.int64(max([np.ceil(dict_match[kreg]["db"]*60./reso_arcmin),minpix]))
                ...
                reso_arcmin=reso*60.
                nxpix=np.int64(dict_match[kreg]["dl"]/reso)
                nypix=np.int64(dict_match[kreg]["db"]/reso)
            a=plt.figure()
            nsub=np.int64(np.max((np.ceil(np.sqrt(nmaps)),1)))
